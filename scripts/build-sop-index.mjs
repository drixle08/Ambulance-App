...
const OUTPUT_PATH = path.join(process.cwd(), "public", "sop-index.json");
...
function chunkText(text, pageNumber) {
  ...
  while (start < text.length) {
    ...
    if (slice) {
      chunks.push({
        id: `sop-page-${pageNumber}-chunk-${chunkIndex}`,
        page: pageNumber,
...
async function extractChunks() {
  ...
  console.log(`Loaded SOP PDF with ${pdf.numPages} pages.`);

  const allChunks = [];

  // Skip pages 1-9 (cover + TOC) — start from page 10 where content begins
  for (let pageNumber = 10; pageNumber <= pdf.numPages; pageNumber += 1) {
...
main().catch((err) => {
  console.error("Failed to build SOP index:", err);
  process.exit(1);
});
