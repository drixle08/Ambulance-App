...
const EXAMPLES = [
  ...
  "Fatigue management requirements for crew?",
  "How is absenteeism managed?",
  "What PPE is required for clinical calls?",
];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^sources?\s*\(sop\)/i.test(trimmed)) {
      nodes.push(
        <div key={keyIdx++} className="mt-3 pt-2 border-t border-slate-700/60 flex items-start gap-1.5">
          <BookOpen className="w-3.5 h-3.5 mt-0.5 shrink-0 text-amber-400" />
    ...
    if (/^\d+\.\s/.test(trimmed)) {
      ...
      if (match) {
        nodes.push(
          <div key={keyIdx++} className="flex gap-2 items-start">
            <span className="mt-0.5 text-xs font-bold text-amber-400 w-5 shrink-0">{match[1]}.</span>
    ...
    // Section heading — ALL CAPS or short title line
    const isHeading =
    ...
    if (isHeading) {
      nodes.push(
        <p key={keyIdx++} className="text-[0.7rem] font-black uppercase tracking-widest text-amber-400 mt-3 first:mt-0">
          {trimmed.replace(/:$/, "")}
        </p>
      );
      continue;
...
export default function SopChatPage() {
  ...
  const handleShare = async (msg: ChatMessage) => {
    if (!canShare) return;
    try {
      await navigator.share({ title: "SOP Chat", text: msg.content });
    } catch (err) {
  ...
  const sendMessage = async (question: string) => {
    ...
    try {
      const resp = await fetch("/api/sop-chat", {
        method: "POST",
      ...
      const sources: SourceDoc[] = Array.isArray(data?.sources)
        ? data.sources.map((s: { id?: unknown; page?: unknown; printedPage?: unknown; sopUrl?: unknown; label?: unknown; code?: unknown }) => ({
            id: String(s?.id ?? crypto.randomUUID()),
            page: Number(s?.page ?? 0),
            printedPage: Number(s?.printedPage ?? 0),
            sopUrl: typeof s?.sopUrl === "string" ? s.sopUrl : "/tools/sop",
            label: typeof s?.label === "string" ? s.label : "SOP",
      ...
      <header className="sticky top-0 z-30 border-b border-slate-800 bg-slate-950/95 backdrop-blur-sm">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0 flex items-center gap-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-amber-500/15 text-amber-400 shrink-0">
              <ClipboardCheck className="w-4 h-4" />
            </span>
            <div>
              <p className="text-[0.65rem] font-semibold tracking-[0.25em] uppercase text-amber-400 leading-none">Reference</p>
              <h1 className="text-sm font-bold text-slate-100 leading-tight">SOP Assistant</h1>
            </div>
          </div>
          {!isEmpty && (
            <button
          ...
          <div className="pt-6 space-y-6">
            <div className="rounded-xl border border-slate-800 bg-slate-900 p-5 space-y-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-100">HMCAS SOP Assistant</p>
                  <p className="text-[0.65rem] text-slate-500">Answers grounded in HMCAS SOP v4.4 (November 2024)</p>
                </div>
              </div>
              <p className="text-sm text-slate-400">
                Ask about operational procedures, HR policies, vehicle management, safety
                requirements, communications, or administrative processes. Answers are based
                only on retrieved SOP content and include source page references.
              </p>
            </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                {EXAMPLES.map((ex) => (
                  <button
                    ...
                    className="text-left rounded-xl border border-slate-800 bg-slate-900 px-3 py-2.5 text-sm text-slate-300 hover:border-amber-500/50 hover:text-slate-100 transition-colors"
                  >
                    {ex}
                  </button>
                ))}
            ...
            {msg.role === "user" ? (
              ...
              <div className="max-w-[95%] space-y-1.5 w-full">
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[0.6rem] uppercase tracking-wider text-amber-500">SOP Assistant</span>
                </div>
                ...
                <div className="flex items-center gap-2 pl-1">
                  ...
                  >
                    {copiedId === msg.id ? (
                      <><Check className="w-3 h-3 text-amber-400" /><span className="text-amber-400">Copied</span></>
                    ) : (
          ...
          <div className="flex justify-start">
            <div className="rounded-2xl rounded-tl-sm bg-slate-900 border border-slate-800 px-4 py-3">
              <div className="flex gap-1.5 items-center h-5">
                <span className="h-2 w-2 rounded-full bg-amber-500/70 animate-bounce [animation-delay:0ms]" />
                <span className="h-2 w-2 rounded-full bg-amber-500/70 animate-bounce [animation-delay:150ms]" />
                <span className="h-2 w-2 rounded-full bg-amber-500/70 animate-bounce [animation-delay:300ms]" />
              </div>
            </div>
          </div>
        )}
        ...
        <div className="max-w-2xl mx-auto px-4 py-3">
          ...
          <form onSubmit={handleSubmit} className="flex items-end gap-2">
            <textarea
              ...
              rows={1}
              placeholder="Ask an SOP question… (Enter to send, Shift+Enter for new line)"
              disabled={isSending}
              className="flex-1 resize-none rounded-xl border border-slate-700 bg-slate-900 px-3 py-2.5 text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-colors disabled:opacity-50 max-h-36 overflow-y-auto"
            />
            <button
              type="submit"
              disabled={isSending || !input.trim()}
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-amber-500 text-slate-950 shadow-md hover:bg-amber-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              aria-label="Send"
            >
              {isSending ? (
          ...
          <p className="text-[0.6rem] text-slate-700 mt-1.5 text-center">
            Answers grounded in HMCAS SOP v4.4 (November 2024) only — always verify with the full document
          </p>
        </div>
      </div>
    </div>
