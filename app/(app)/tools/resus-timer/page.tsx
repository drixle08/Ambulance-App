...
export default function ResuscitationTimerPage() {
  ...
  const handleResetClick = () => {
    if (!showConfirmReset) {
      ...
      resetTimeoutRef.current = setTimeout(() => setShowConfirmReset(false), 3000);
    } else {
      if (resetTimeoutRef.current) clearTimeout(resetTimeoutRef.current);
      ...
      <main className="mx-auto flex min-h-screen max-w-lg flex-col gap-5 px-4 pb-6 pt-4">
        ...
        <section className="mt-1 flex gap-3">
          ...
          <button
            ...
            aria-label={showConfirmReset ? "Confirm reset all data" : "Reset timer"}
            className={`h-16 min-w-32 rounded-3xl border transition-all shadow-md active:translate-y-[1px] text-base font-black ${
              showConfirmReset
