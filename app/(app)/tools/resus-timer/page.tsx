...
export default function ResuscitationTimerPage() {
  ...
  // Cleanup reset confirmation timeout
  useEffect(() => {
    return () => {
  ...
  }, [handleReset, performReset, handleStartPause, voiceEnabled, voiceSupported]);
      ...
      <main className="mx-auto flex min-h-screen max-w-lg flex-col gap-5 px-4 pb-6 pt-4">
        ...
        <section className="mt-1 flex gap-3">
          ...
          <button
            type="button"
            onClick={handleReset}
            className={`h-16 min-w-28 rounded-3xl border text-base font-black shadow-md active:translate-y-[1px] transition-colors ${
              showConfirmReset
